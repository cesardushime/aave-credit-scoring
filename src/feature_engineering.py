...
def generate_user_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Lowercase action_type for consistency
    ...
    # Combine deposit and borrow into a single DataFrame, fill missing with 0
    ...
    ratio_df['borrow_deposit_ratio'] = np.where(
        ratio_df['total_deposit_usd'] == 0,
        0,
    ...
    def avg_time_between_txn(user_df):
        ...
        return deltas.mean().total_seconds() / 3600 if len(deltas) > 0 else 0
